...
药材为半径 $R=2\ \mathrm{cm}$、长度 $L=25\ \mathrm{cm}$ 的圆柱。题目要求给出距中心的分布，本问将其解释为中截面的径向分布，采用轴对称、仅沿径向传输的一维近似。该处理与圆柱物料干燥中常用的 Fourier 导热—Fick 扩散模型一致 [1-3]。二维模型可刻画端部效应，也可另行假定端面与侧面使用相同传递系数；题目未区分端面参数并不意味着二维模型不可建立。本次未完成二维对照，故一维近似的轴向误差仍未量化 [4]。
