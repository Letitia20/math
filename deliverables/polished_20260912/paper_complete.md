...
本参赛队在竞赛过程中使用了AI工具，主要用于代码调试、重复性计算检查、文件一致性检查和语言润色，详细使用情况见支撑材料。

本题的核心建模与分析由参赛队主导：队员确定模型框架、假设、参数和边界条件，审定数值结果及其解释，并决定最终提交内容。AI 的任何建议、代码修改或文字均经队员逐项审查、修改和核验。参赛队对模型、代码、数值、参考文献和最终表述承担责任。工具的具体用途与核验方式见支撑材料中的《AI 工具使用详情.pdf》，本节披露不作为参考文献引用。
