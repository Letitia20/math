...
药材烘干时，热量由表面传入，水分由内部向外迁移，二者的变化速度不同；物性变化和几何收缩也会影响干燥终点。针对这一过程，本文建立圆柱形药材的一维径向传热传质模型，采用环形有限体积法和隐式时间积分进行求解，并用 Richardson 外推提高计算精度。
...
题目研究一根长 25 cm、初始半径为 2 cm 的圆柱形药材。烘干开始时，药材温度为 28 ℃，干基含水率为 2.55 kg/kg。附件 1 给出了烘房初期的温度和含湿量，附件 2 给出了药材半径的变化。题目既要求考察整体干燥情况，又要求给出指定位置的温度和含水率，因此需要建立能够描述内部变化的模型 [1]。
...
考虑到题目既要求药材内部的局部分布，又要求给出全域达标时间，只用平均含水率曲线不能满足要求。本文采用 Fourier 导热方程描述传热过程，采用 Fick 有效扩散方程描述水分迁移，再用表面对流边界联系药材和烘房环境。圆柱物料的热质耦合模型 [2] 和非等温移动边界模型 [3] 为建模提供了参考，物性和观测数据均采用题目给定值。

药材长径比为 6.25，因此本文只考虑径向变化，并将结果看作圆柱中截面的分布。建模时，先处理常物性情形，再考虑物性变化，最后加入实测收缩。问题四直接使用附件给出的半径数据，不再另外拟合收缩方程。
...
附件 1 只记录了前 4 h 的烘房环境，而问题三、四需要更长时间的环境条件。具体做法是：取最后一小时 61 个观测点的均值作为后续环境平台。该时段温度和含湿量的均值分别为 49.998934 ℃和 0.04998754 kg/kg，标准差分别为 0.154353 ℃和 0.00015013 kg/kg；相应线性趋势约为 0.002824 ℃/h 和 −0.00001288 kg/(kg·h)。可以看到，该时段波动较小。第 8.3 节再比较不同延拓方法对结果的影响。

### 3.2 环形有限体积离散

为计算温度和含水率在半径方向上的变化，将药材截面划分为若干同心圆环。省略各单元共有的轴向长度后，控制体体积为 $V_i=\pi(r_{i+1/2}^2-r_{i-1/2}^2)$，外侧面积为 $A_{i+1/2}=2\pi r_{i+1/2}$。对一般传输变量 $u$，记蓄积系数为 $s$、输运系数为 $\Gamma$，可得
...
空间离散后，原方程变为常微分方程组。本文采用后向差分公式（BDF）进行隐式积分。为避免出现负含水率，先令 $z=\ln C$，按 $z_t=C_t/C$ 计算，再由 $C=\exp z$ 还原含水率。积分器自动调整步长，并在题目要求的时刻输出结果。
...
将附件 1 的环境数据代入上述模型，得到表 1、表 2。表中列出题目指定时刻和位置的结果。完整的 1—1800 s 温度场和含水率场保存在支撑材料 result1.xlsx 中，径向间隔为 0.1 cm，表中结果按题意保留四位小数。
...
将式（8）—（9）代入式（5），再结合初值和式（7）的边界条件，即可得到问题二模型。含水率变化会改变密度、比热和导热系数；温度又会通过扩散率影响水分迁移，因此需要同时求解温度和含水率。计算 Arrhenius 项时，将摄氏温度换算为开尔文温度。
...
问题三要求药材各处的含水率都低于 0.15 kg/kg，因此不能只看表面值或平均值。具体计算时，遍历全部径向节点，以最大含水率判断是否达标。结果表明，中心位置的含水率最高，所以干燥终点由中心位置决定。在临界时刻有 $g(t_*)=0$，其后的时刻才严格低于阈值。

为确定终点，在阈值两侧对各径向位置的未舍入含水率进行时间插值，取最晚达到阈值的位置。若设备只能按整分钟停止，则取临界时刻之后的第一个整分钟，即
...
问题四中药材半径随时间变化。附件 2 已经给出 $R(t)$，因此直接把实测半径代入模型。在均匀径向收缩、长度不变的条件下，材料速度为 $v_r=(\dot R/R)r$。设当前干物质体积密度为 $\rho_d$，相对骨架的水分通量为 $\mathbf j_w$，则
...
初始干密度均匀，截面又作均匀径向收缩，因此干物质守恒给出 $\rho_d(t)R(t)^2=\rho_{d0}R_0^2$。此时干密度只随时间变化，式（13）中的 $\rho_d$ 可以约去，得到
...
为便于计算，令 $\xi=r/R(t)$，将收缩区域变换到固定区间 $0<\xi<1$。固定 $\xi$ 的网格速度与骨架速度相同，因此坐标变换项与式（14）中的对流项相互抵消，得到
...
题目要求给出固定位置的含水率，因此每个时刻都用 $\xi=r/R(t)$ 找到相应的计算位置。当 $r>R(t)$ 时，该位置已经在药材之外，工作簿中的单元格留空；“药材表面”一列始终取 $\xi=1$，随边界移动。
...
利用前 600 s 的逐秒输出和后续分钟输出，对边界通量进行积分。第三问两级网格的相对最大残差约为 1.21×10⁻⁶，第四问分别约为 1.61×10⁻⁶ 和 1.69×10⁻⁶。残差很小，说明计算结果满足模型中的水分通量平衡。
...
为检验密度关系与收缩假设是否相容，将附录 4 的 $\rho(C)$ 看作当前真实湿基密度。此时有 $\rho_d=\rho(C)/(1+C)$，总干物质量为

$$
M_d(t)=2\pi L R(t)^2\int_0^1\frac{760+90C(\xi,t)}{1+C(\xi,t)}\xi\,\mathrm d\xi. \qquad (22)
$$

对非均匀含水率场积分，在 1280 区间的临界时刻得到 $M_d(t_*)/M_d(0)\approx0.890289$，偏差约为 10.97%。这说明，若把给定密度看作真实湿基密度，长度不变的实测收缩与干物质守恒不能同时严格成立。该偏差用于检验模型关系，并不表示药材实际损失了相同比例的干物质。
