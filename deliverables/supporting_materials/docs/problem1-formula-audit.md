...
- 复查发现早期表面边界层是最难收敛区域，仅检查 100 s 以后的 35 个论文节点不足以保证完整工作簿的四位小数稳定。
- 对完整的 1800×21 个输出节点，5120 与 10240 个半径区间的最大差异为：温度
  \(1.03\times10^{-6}\ ^\circ\mathrm C\)，水分浓度
  \(4.57\times10^{-6}\ \mathrm{kg/kg}\)。
- 正式结果采用上述两级网格的二阶 Richardson 外推。由此，100 s、2 cm 的水分浓度从 2.2471 修正为 2.2470，1 s、2 cm 从 2.5178 修正为 2.5177。
