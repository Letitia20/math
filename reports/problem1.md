...
将 $[0,R]$ 等分为 1280 个径向区间，在节点周围构造同心环形控制体。对一般变量 $u$ 及输运系数 $\Gamma$，第 $i$ 个控制体的半离散形式为

\[
V_i\frac{\mathrm du_i}{\mathrm dt}
...
其中 $V_i=\pi(r_{i+1/2}^2-r_{i-1/2}^2)$，界面系数采用调和平均。圆心控制体的内表面积自然为零，避免直接计算方程中的 $1/r$ 奇点；表面控制体直接加入 Robin 边界通量。时间方向使用适合刚性扩散方程的 BDF 隐式积分，并以 $\log C$ 为水分状态量以保证 $C>0$。最后把数值解插值到 $r=0,0.1,\ldots,2.0\ \mathrm{cm}$，并按题意保留四位小数。
...
温度由外向内逐渐升高。1800 s 时表面温度为 $36.7856\ ^\circ\mathrm C$，中心温度为 $33.5753\ ^\circ\mathrm C$，径向温差约 $3.2103\ ^\circ\mathrm C$。水分变化则更集中于表层：1800 s 时表面浓度降至 $1.5102\ \mathrm{kg/kg}$，中心仍约为 $2.5500\ \mathrm{kg/kg}$。这是因为在当前 $C$ 范围内水分扩散系数约为 $10^{-9}\ \mathrm{m^2/s}$ 量级，30 min 内的扩散影响深度远小于半径；水分曲线出现陡峭边界层，而温度场相对平滑。
...
3. **网格收敛。** 在 $t=100,300,\ldots,1800\ \mathrm s$ 及 $r=0,0.5,1.0,1.5,2.0\ \mathrm{cm}$ 上比较 640 与 1280 个径向区间，最大绝对差为 $2.00\times10^{-6}\ ^\circ\mathrm C$ 和 $2.18\times10^{-5}\ \mathrm{kg/kg}$，均小于结果保留四位小数所需的量级。
